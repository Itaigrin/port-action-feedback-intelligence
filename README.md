...
**The count is also the drill-down.** Pressing *"46 negative feedback records"* scopes the evidence section to exactly those 46 records and jumps to it; pressing it again clears the scope. It stays a count rather than turning into a button label when selected — the number is the card's finding, so swapping it for *"Showing below"* would cost the reader the fact to gain nothing, and the selected state is carried by an inverted badge instead. Scoping is by the ids the badge counted, never by re-deriving "everything in this stage", which would pull back the non-negative records the card deliberately excluded and leave the section showing more than the badge that opened it claimed.

**Exactly one selection scopes the evidence section at a time.** The two cards and every product action's *"View supporting feedback"* all point the same section at a different set, so selecting any one of them clears the others. Two live selections would leave the section showing one of them while both read as active on screen.

**The focus line names what is concentrated here, not just what is common.** Feature gap is 72% of all negative feedback and ranks first in *every* journey stage, so a line built on the most frequent problem type says the same sentence on every card — true each time, and useless for telling the stages apart. The line therefore reports two facts: what most of the volume is, then which problem type is over-represented **against the records outside this group** (not against the whole dataset, which would include the group in its own baseline and dampen the very signal being measured). Permissions & approvals reads *"Mostly feature gap. Unusually concentrated here: configuration complexity."*

A type is only called distinctive if it has **at least 3 records and at least 1.5x the outside rate** — but that count and ratio decide the claim, they never appear on the card. "(3 records, 4.3x elsewhere)" is a stat a reader has to go verify, not a claim they can just read; the card states the finding in plain language instead. A two-record spike produces a spectacular ratio and means nothing: the subcategory card's highest-lift type sits at 7.8x on two records, and the card says nothing stands out rather than headlining it. When nothing clears both bars the line says so; when the reader has filtered to a single group there is no baseline left, so it falls back to frequency alone rather than claiming a comparison it cannot make. A type absent everywhere else is reported as *"seen nowhere else"* — the one exception, because it is a qualitative fact rather than a number.
...
**Date window:** the rolling three months ending on the date this analysis was generated (`meta.generated_at`), not on whichever day a reader happens to open the dashboard. This is a static POC snapshot — nothing re-collects it as real weeks pass — so anchoring "recent" to the actual wall clock would make every trend figure quietly shrink and eventually vanish purely because the calendar moved on without the data doing the same, which is staleness pretending to be a finding. The number only moves again if the pipeline is rerun. As a second layer under that: if the week that would be "last completed" as measured from the analysis date could not possibly contain any data, the window falls back further, to the newest known `created_at`, and the chart says so — rendering an empty chart would look like a bug rather than a property of the data.
