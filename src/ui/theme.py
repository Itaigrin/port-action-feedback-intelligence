...
/* The count badge is also the control that scopes the evidence section to
   these records. It stays a badge rather than becoming a button: the number
   is the card's finding, and dressing it as a button would read as "here is
   an action" instead of "here is what we found, and you can go see it". */
/* :visited is spelled out on both states for the same reason .afi-action-btn
   spells it out: this is an <a href="#">, which the browser counts as
   visited the moment the page loads, so a state whose colour is only set by
   the unvisited rule would repaint itself on first render. */
/* !important on colour, like .afi-action-btn below: Streamlit ships a
   `.st-emotion-cache-<hash> a` rule that paints every anchor inside a
   markdown block its own link blue, and one class plus an element beats one
   class, so this badge rendered blue without it. The hash is autogenerated
   and changes between releases, so it cannot be targeted -- winning on our
   own selector is what survives an upgrade. */
.afi-insight-count-btn, .afi-insight-count-btn:visited {{
  text-decoration: none !important; cursor: pointer;
  color: var(--red) !important;
}}
.afi-insight-count-btn:hover {{ filter: brightness(.94); }}
/* Selected: inverted rather than merely tinted, so which of the two cards is
   scoping the section below is readable at a glance from across the row. */
.afi-insight-count-btn.is-selected,
.afi-insight-count-btn.is-selected:visited {{
